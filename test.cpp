...
{                               //for thesis
  ...
  VALUE narrowa = 10;
  VALUE narrowb = 20;
  ___CODEUNIT (tree.visited = 0; ___.tic ("minmax"); a = tree.minmax (p); ___.toc (); ___COUT1 (tree.visited););

  // 当界值不当，且有浅层叶结点时返回值可能越界，
  // 层数稍深，返回值会被抹平，则返回原始界值
  ___CODEUNIT (tree.visited = 0; ___.tic ("alphabeta_minmax"); b = tree.alphabeta_minmax (p, widea, wideb); ___.toc (); ___COUT1 (tree.visited););
...
*/
  ...
  ___CODEUNIT (tree.visited = 0; ___.tic ("negamax"); u = tree.negamax (p); ___.toc (); ___COUT1 (tree.visited););
  ___CODEUNIT (tree.visited = 0; ___.tic ("alphabeta_negamax"); v = tree.alphabeta_negamax (p, newwidea, newwideb); ___.toc (); ___COUT1 (tree.visited););
  ___CODEUNIT (tree.visited = 0; ___.tic ("failsoft_negamax"); w = tree.failsoft_negamax (p, newwidea, newwideb); ___.toc (); ___COUT1 (tree.visited););

  // 当兄弟结点无序时搜索的结点往往更多
  ___CODEUNIT (tree.visited = 0; ___.tic ("negascout"); x = tree.negascout (p, newwidea, newwideb); ___.toc (); ___COUT1 (tree.visited););
  ___CODEUNIT (tree.visited = 0; ___.tic ("mtdf"); y = tree.mtdf (p); ___.toc (); ___COUT1 (tree.visited););
...
teststl ()
{
  ...
  ___COUT6 (mapm[1], mapm[3], mapm[4], mapm.size (), v.size (), sets.size ());
