...
bool
TARGET::operator< (const TARGET & t) const
{
...
TARGET::operator == (const TARGET & t) const
{
  return (aim == t.aim && obj == t.obj && env == t.env && probvalue == t.probvalue && sentewish == t.sentewish && passtimes == t.passtimes);
}
...
BITB
TARGETSEARCH::scope (INFOGO * infop, TARGET tg)
{
  BITB tmp;
  switch (tg.aim)
    {
...
bool
TARGETSEARCH::isleaf (INFOGO * infop, TARGET tg)
{
  switch (tg.aim)
    {
    case ROOT:
...
RESULT
TARGETSEARCH::leafvalue (INFOGO * infop, TARGET tg)
{
  RESULT rs;
  switch (tg.aim)
    {
...
bool
TARGETSEARCH::isleaf (INFOGO * infop, TARGETS tgs)
{
  bool b, bb;
  b = 0;
  for (int i = 0; i < tgs.nut.size (); ++i)
...
RESULT
TARGETSEARCH::leafvalue (INFOGO * infop, TARGETS tgs)
{
  RESULT r, rr;
  r = upbound ();
  for (int i = 0; i < tgs.nut.size (); ++i)
...
INFOGO *
TARGETSEARCH::findson (INFOGO * infop, MOVE mv)
{
...
bool
TARGETSEARCH::ismax (INFOGO * infop, TARGETS tgs)
{
  COLOR trn = tgs.nut[0][0].passtimes ? infop->getlastclr () : infop->turn ();
  return trn == BLACK;
}
...
TARGETS
TARGETSEARCH::control (INFOGO * infop, TARGETS tgs)
{
...
RESULT
TARGETSEARCH::recordpath (RESULT result, MOVE move)
{
  result.move = move;
  return result;
}
