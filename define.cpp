...
void
___parassert (bool exp, string s)
{
  if (!exp)
    {
...
void
___assert (bool exp, string s, string _f_, long _l_)
{
  if (!exp)
    {
...
void
TIMER::tic (string s, int n)
{
  cout << std::setw (20) << std::left << s << std::setw (10) << std::right << n << " times :";
  ticat = clock ();
}
...
POS
INIT::getp (int i) const
{
  return i2p[i];
}

VP
INIT::getnear4 (int i) const
{
  return i2near4[i];
}

int
INIT::geti (const POS & p)
{
  return p2i[p];
}

VP &
INIT::getnear4 (const POS & p)
{
  return p2near4[p];
}

void
INIT::setzobristmask (int width)
{
...
bool
INIT::load_xorand_key2rand (string filename)
{
...
bool
INIT::save_xorand (string filename)
{
...
int
INIT::getq8 (int i) const
{
  return q8[0][i];
}

KEY
INIT::getrand (const POS & pos, COLOR clr)
{
  if (clr == BLACK)
