...
BLOCK
BITB::blockat (const POS & pos) const
{
...
BITB
BITB::blockon_ (const BITB & t) const
{
...
BITB
BITB::blockon (const BITB & t) const
{
...
bool
BITB::notbigger (int x, int y) const
{
  pair < int, int >rg = range ();
  return ((rg.first <= x && rg.second <= y) || (rg.first <= y && rg.second <= x));
}

BITB
BITB::ruboff (int x, int y) const
{
  BITB b = *this;
  ITR itr = ITR (b);
...
BITB
BITB::ruboff (const BITB & b) const
{
  return *this ^ (*this & b);
}

BITB
BITB::close (int t) const
{
  BITB bb = *this;
  for (int i = 0; i < t; ++i)
...
bool
BITB::touch_ (const BITB & bb) const
{
  return (((*this) & bb) == NULL_BB) && (((*this) & bb.dilate (1)) != NULL_BB);
}

bool
BITB::diagonal_touch_ (const BITB & bb) const
{
  return (((*this) & bb) == NULL_BB) && (((*this) & bb.diagonal ()) != NULL_BB);
}
...
bool
BITB::ismassof (const BITB & big) const
{
...
bool
BITB::touchboarder () const
{
  return (*this & frame (star (CENTER), MIDBS)) != NULL_BB;
}

bool
BITB::contain (const BITB & c) const
{
  return (*this & c) == c;
}
