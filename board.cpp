...
GO::GO ():xx (NULL_BB), oo (NULL_BB), hotko (NULL_POS), history (0), lastpos (NULL_POS), lastclr (WHITE), kill (NULL_BB)
{
}

GO::GO (const BITB & x, const BITB & o, const BITB & d, const POS & k, const POS & l, COLOR c, int h):xx (x), oo (o), kill (d), hotko (k), lastpos (l), lastclr (c), history (h)
{
}

GO::GO (const PBB & pbb, const BITB & d, const POS & k, const POS & l, COLOR c, int h): xx (pbb.first), oo (pbb.second), kill (d), hotko (k), lastpos (l), lastclr (c), history (h)
{
}

GO::GO (const GO & go): xx (go.xx), oo (go.oo), hotko (go.gethotko ()), history (go.gethistory ()), lastpos (go.getlastpos ()), lastclr (go.getlastclr ()), kill (go.getkill ())
{
}

void
GO::random (MODE_RANDOMBOARD mode)
{
  switch (mode)
    {
    case ILLEGALIZE:
...
void
GO::add (const POS & pos, COLOR clr)
{
...
GO::transpose () const
{
  return GO (xx.transpose (), oo.transpose (), kill.transpose (), transp (hotko), transp (lastpos), lastclr, history);
}

GO
GO::rotate (int angle) const
{
  return GO (xx.rotate (angle), oo.rotate (angle), kill.rotate (angle), rot (angle, hotko), rot (angle, lastpos), lastclr, history);
}

COLOR
GO::color (const POS & pos) const
{
...
BLOCK
GO::expand (const BLOCK & b, COLOR clr) const
{
  BITB ex;
...
BITB
GO::enemy_ (const BLOCK & blk, COLOR clr) const
{
...
// 与 libertysite() 功能互补
BITB
GO::enemysite (const BLOCK & blk, COLOR clr) const
{
  ___PARASSERT (blk.ismassof (getbb (clr)));
...
BITB
GO::libertysite (const POS & pos) const
{
  COLOR clr = color (pos);
...
/* 展开版之 movealbe() */
bool
GO::moveable_ (const POS & pos, COLOR clr) const
{
  ___PARASSERT (!outside (pos) && (clr == BLACK || clr == WHITE));
  if (color (pos) != EMPTY)
    return 0;
...
/* 未展开版 moveable() */
bool
GO::moveable (const POS & pos, COLOR clr) const
{
  ___PARASSERT (!outside (pos) && (clr == BLACK || clr == WHITE));
  if (color (pos) != EMPTY)
    return 0;
...
/* 调用版 move() */
bool
GO::move_ (const POS & pos, COLOR clr)
{
  ___PARASSERT (!outside (pos) && (clr == BLACK || clr == WHITE));
  if (!moveable (pos, clr))
    return 0;
...
/* 展开之独立版 move() */
bool
GO::move__ (const POS & pos, COLOR clr)
{
  ___PARASSERT (!outside (pos) && (clr == BLACK || clr == WHITE));
  if (color (pos) != EMPTY)
    return 0;
  ...
  if (cN == EMPTY || cS == EMPTY || cE == EMPTY || cW == EMPTY)
    mvable = 1;
  // 可融合成有气的块
  if (((cN == clr) && !onlyN) || ((cS == clr) && !onlyS) || ((cE == clr) && !onlyE) || ((cW == clr) && !onlyW))
    mvable = 1;
  // 可提
  if ((cN != clr) && onlyN)
    {
...
/* 独立版 move() */
bool
GO::move (const POS & pos, COLOR clr)
{
  ___PARASSERT (!outside (pos) && (clr == BLACK || clr == WHITE));
  if (color (pos) != EMPTY)
    return 0;
...
PUU
GO::pemis3232 (const BITB & patmask) const
{
  ...
  PUU t;
  PUU R;
  t.first = a.first * b.second + a.second * b.first + mulhigh32 (a.second, b.second);
  t.second = a.second * b.second;
  R.first = t.first * c.second + t.second * c.first + mulhigh32 (t.second, c.second);
  R.second = t.second * c.second;
  return R;
}

ULL
GO::pemis64 (const BITB & patmask) const
{
  return (xx & patmask).pemis64 () * (oo & patmask).pemis64 () * ((xx | oo) & patmask).pemis64 ();
}

COLOR
GO::operator[] (const POS & pos) const
{
  return color (pos);
}

GO
GO::operator() (const BITB & mark, const POS & markp) const
{
  GO tmp = *this;
  tmp.kill = mark;
  tmp.hotko = markp;
