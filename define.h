...
enum MODE_PROMPT { BEGINGAME, PREMOVE_MAN, AFTERMOVE_MAN, PREMOVE_PC, AFTERMOVE_PC, ENDGAME, BITSET };
enum MODE_PLAY { MAN_PC, PC_PC, MAN_MAN };
enum MODE_RANDOMEYE { LESS_LESS, BIGGER_LESS, LESS_LESS_CENTER, EQ_EQ };
enum MODE_RANDOMBOARD { LEGALIZE, ILLEGALIZE };
enum DIRECTION { EAST = 0, SOUTH = 1, WEST = 2, NORTH = 3, ES, EN, WS, WN, CENTER, DIRECTION_SIZE };
enum COLOR { BLACK, WHITE, EMPTY, BLACK_WHITE, HOTKO, NULL_CLR, COLOR_SIZE };
enum ACTION { AFTERDELETE, KILL, RESCUE, ACTION_SIZE };
enum EYEKIND { ENEMY_ALIVE, FALSEEYE, ONEEYE, CO_ONEEYE, CO_SOMEEYE, TWOEYE, SOMEEYE, MANYEYE, UNKOWN_WHENPASS };
enum ROUTE { R3 = 0, R4, R5, R4UP, R4DOWN, R4DOWN_EAST, R4DOWN_SOUTH, R4DOWN_WEST, R4DOWN_NORTH };
enum STATE { ALIVE, DEAD, UNCLEAR };
const string eyekindname[] = { "ENEMY_ALIVE", "FALSEEYE", "ONEEYE", "CO_ONEEYE", "CO_SOMEEYE", "TWOEYE", "SOMEEYE", "MANYEYE", "UNKOWN_WHENPASS" };
...
___cout (T t, string s)
{
  if (typeid (T) == typeid (int) || typeid (T) == typeid (unsigned) || typeid (T) == typeid (float) || typeid (T) == typeid (double) || typeid (T) == typeid (ULL) || typeid (T) == typeid (POS)
      || typeid (T) == typeid (PUU) || typeid (T) == typeid (PII))
    cout << std::setw (20) << std::left << setx (G) << (s + ": ") << setx (W) << t << endl;
  else
    cout << setx (G) << s << ": " << endl << setx (W) << t << endl;
}
