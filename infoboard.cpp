...
BITB
INCREMENT::getcontent (int i) const
{
  return content[tag[i]];
}

BITB
INCREMENT::getenemysite (int i) const
{
  return enemysite[tag[i]];
}

BITB
INCREMENT::getlibertysite (int i) const
{
  return libertysite[tag[i]];
}

COLOR
INCREMENT::getcolor (int i) const
{
  return color[i];
}

int
INCREMENT::getliberty (int i) const
{
  return liberty[tag[i]];
}

int
INCREMENT::gettag (int i) const
{
  return tag[i];
}

void
INCREMENT::setcontent (int i, const BITB & blk)
{
  content[tag[i]] = blk;
}

void
INCREMENT::setcolor (int i, COLOR clr)
{
  color[i] = clr;
}

void
INCREMENT::setenemysite (int i, const BITB & bb)
{
  enemysite[tag[i]] = bb;
}

void
INCREMENT::setlibertysite (int i, const BITB & bb)
{
  libertysite[tag[i]] = bb;
}

void
INCREMENT::setliberty (int i, int n)
{
  liberty[tag[i]] = n;
}

void
INCREMENT::settag (int i, int t)
{
  tag[i] = t;
}
...
bool
INCREMENT::operator == (const INCREMENT & ic) const
{
...
void
ZOBRIST::setzobrist (int i, const POS & lastpos, COLOR lastclr, const BITB & kill)
{
...
KEY
ZOBRIST::getzobrist (int i) const
{
  return key[i];
}

int
ZOBRIST::getcount (int i) const
{
  return count[i];
}
...
PATFREQ::setflag ()
{
  for (int i = 0; i < BS * BS; ++i)
    for (int j = 0; j < PEMIS_KIND; ++j)
      x[j][i] = NOINIT, o[j][i] = NOINIT;
}

void
PATFREQ::setf (const POS & pos, COLOR clr, int i, int freq)
{
  if (clr == BLACK)
    x[i][pos2i (pos)] = freq;
...
int
PATFREQ::getf (const POS & pos, COLOR clr, int i) const
{
  if (clr == BLACK)
...
bool
INFOGO::operator == (const GO & go) const
{
  return (go.xx == xx && go.oo == oo && go.getkill () == getkill () && go.gethotko () == gethotko () && go.getlastpos () == getlastpos () && go.getlastclr () == getlastclr () && go.gethistory () == gethistory ());
}
  ...
  for (POS p = itr.posbegin (); !itr.posend (); p = itr.posnext ())
    {
      ...
      int tag = blk.tag () == NULL_POS ? NULL_TAG : pos2i (blk.tag ());
      int m = pos2i (p);
      tmp.settag (m, tag);
...
bool
INFOGO::move (const POS & pos, COLOR clr)
{
  int ip = pos2i (pos);
  ...
  if (kill.count () == 1 && inc.getcontent (ip).count () == 1 && inc.getliberty (ip) == 1)
    {
...
KEY
INFOGO::getzobrist (int n) const
{
  return zobrist.getzobrist (n);
}

VI
INFOGO::getf_zobrist (int n, const BITB mask, COLOR clr) const
{
  VI vi (BS * BS, 0);
  MOVE move;
...
*/
VI
INFOGO::getf_patfreq (int n, const BITB mask, COLOR clr)
{
  VI vi (BS * BS, 0);
  PUU puu;
  INFOGO snap = *this;
...
int
INFOGO::samemassmapsnumb () const
{
  int m = min (father->area.massmaps.size (), area.massmaps.size ());
  for (int i = 0; i < m; ++i)
    if (father->area.massmaps[i].xx == area.massmaps[i].xx && father->area.massmaps[i].oo == area.massmaps[i].oo)
      return i;
  return -1;
}
...
// 返回所有钮形区域，可能有些区域不止四子
BITB
INFOGO::cutingpos () const
{
  BITB tmpx, tmpo, tmp;
  tmpx.r[0] = xx.r[0] & xx.r[1] << 1 & oo.r[0] << 1 & oo.r[1] | xx.r[0] & xx.r[1] >> 1 & oo.r[0] >> 1 & oo.r[1];
  tmpo.r[0] = oo.r[0] & oo.r[1] << 1 & xx.r[0] << 1 & xx.r[1] | oo.r[0] & oo.r[1] >> 1 & xx.r[0] >> 1 & xx.r[1];
...
BITB
INFOGO::cutpos (COLOR sente) const
{
  INFOGO tmp = *this;
  switch (sente)
    {
...
/* class AREA */
PBC
AREA::getmass (const BITB & mask, COLOR clr) const
{
...
PBC
AREA::getcluster (const BITB & mask) const
{
...
PBC
AREA::getpotential (const BITB & mask) const
{
  ...
  // 否则写入“适当部分”massmap　。
void
AREA::setclustermap (const BITB & mask, COLOR clr)
{
  switch (clr)
    {
    case BLACK:
...
BITB
AREA::getclustermap (COLOR clr, STATE st) const
{
  BITB unclear = clustermap.xx & clustermap.oo;
  switch (clr)
    {
...
AREA::delta () const
{
  ...
  //　注意可能有双方未定区相挨，不影响此算法
bool
AREA::isolated (const PBC & pbc) const
{
  switch (pbc.second)
    {
    case BLACK:
...
STATE
AREA::clusterstate (const PBC & pbc) const
{
...
bool
AREA::onland (const PBC & pbc, const POS & pos) const
{
...
void
AREA::setworknumb (const POS & pos)
{
...
bool
AREA::newmass (const POS & pos, COLOR clr) const
{
