...
POS
INOUT::in2pos (const POS & pos, string in)
{
  POS p = ai2pos (in);
  if (p != NULL_POS)
    return p;
  ...
  if (isdigit (in[0]) && (in.size () == 1))
    in += "++00";
  if ((in.find ("++") == in.npos) && (in.find ("-+") == in.npos) && (in.find ("--") == in.npos) && (in.find ("+-") == in.npos))
    return NULL_POS;
  if (!isdigit (in[0]))
    in.insert (0, "0");
  op = in.substr (1, 2);
...
POS
INOUT::jj2pos (string in, const POS & pp)
{
  string op;
  int n0, n3, n4;
  n0 = in[0] - '0';
...
void
INOUT::print (const GO & go, const BITB & mask)
{
  string s;
  ITR itr;
...
void
INOUT::print (const GO & go, const GO & markgo, RGB xxc, RGB ooc, RGB xoc)
{
  string s;
  ITR itr;
  for (POS pos = itr.ioposbegin (); !itr.ioposend (); pos = itr.ioposnext ())
...
VM
INOUT::sgf2move (string filename)
{
...
void
INOUT::infop2sgf (INFOGO * infop, string filename)
{
  while (infop->father)
    infop = infop->father;
  string tmp = "(";
...
GO
INOUT::frommatlab (string filename)
{
  ifstream file (filename.c_str ());
  if (!file)
    {
...
VS
INOUT::getfilename (string path, string type)
{
  ...
  while ((p = names.find_first_of ("\n")) != names.npos)
    {
      tmps = names.substr (0, p), names.replace (0, p + 1, ""), vecs.push_back (tmps);
    }
  system ("rm -f tmp");
  return vecs;
}
...
{
  cout << GO (info.xx, info.oo, info.getkill (), info.gethotko (), info.getlastpos (), info.getlastclr (), info.gethistory ());
  return os;
}
...
ostream & operator<< (ostream & os, const PAT & pat)
{
  INOUT inout;
  inout.axisshow = 0;
  inout.print (GO (pat.first.xx & pat.second, pat.first.oo & pat.second), pat.second.project ());
  return os;
}
